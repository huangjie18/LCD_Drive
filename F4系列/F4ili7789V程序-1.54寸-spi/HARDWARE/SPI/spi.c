...
void SPI1_Init(void)
{
    ...
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA | RCC_AHB1Periph_GPIOB |
                           RCC_AHB1Periph_GPIOC | RCC_AHB1Periph_GPIOG, ENABLE); //使能引脚时钟
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_SPI1, ENABLE);//使能SPI1时钟


    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;//DC引脚
    ...
    //GPIOA5,7初始化设置
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5 | GPIO_Pin_7; //PA引脚复用功能输出
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;//复用功能
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//20ns翻转
    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
    ...
    //分频值不能太小，数值越小，时钟线速度越快，需要和屏的时钟速度对应才不会乱码
    //有的屏时钟不能太小，2分频时会错误
    ...
    SPI_InitStructure.SPI_BaudRatePrescaler =
        SPI_BaudRatePrescaler_2;      //定义波特率预分频的值:波特率预分频值
...
u8 SPI1_ReadWriteByte(u8 TxData)
{
    ...
    while (SPI_I2S_GetFlagStatus(SPI1,
                                 SPI_I2S_FLAG_TXE) == RESET) {} //等待发送区空

    SPI_I2S_SendData(SPI1, TxData); //通过外设SPIx发送一个byte  数据

    for(i = 0; i < 11;i++); //防止数据传输太快，导致数据错误，i=33时大约等于下面语句的延时时间,1个=50us延时
    //数值越小，数据间隔越小，如果这次数据还没传输完下一个数据就来了，导致数据交错，屏会显示错误
...
void SPI1_Init(void)
{
	...
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOA|RCC_AHB1Periph_GPIOB|
	RCC_AHB1Periph_GPIOC|RCC_AHB1Periph_GPIOG, ENABLE);//使能GPIOA,GPIOB,GPIOC,GPIOG时钟
	
	
	//GPIOA5,A7初始化设置
	  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5|GPIO_Pin_7;//SCL和SDA对应IO口
	  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;//普通输出模式
	  GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;//推挽输出
	  GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
	  GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
