...
#include "GUIDebug.h"
#include "ili7735r.h"      //LCD底层驱动头文件

//#if (LCD_CONTROLLER == -1) \
//    && (!defined(WIN32) | defined(LCD_SIMCONTROLLER))
#if (LCD_CONTROLLER == -1)   //使用自己的LCD驱动
/*********************************************************************
*
*       Defines
*
...
//        LCD_L0_SetPixelIndex(x++, y, (*p & (0x80 >> Diff)) ? Index1 : Index0);
		LCD_Fast_DrawPoint(x++, y, (*p & (0x80 >> Diff)) ? Index1 : Index0);	  
		  if (++Diff == 8) {
          Diff = 0;
				  p++;
			  }
...
//          LCD_L0_SetPixelIndex(x, y, LCD_NUM_COLORS - 1 - Pixel);
			  LCD_Fast_DrawPoint(x, y, LCD_NUM_COLORS - 1 - Pixel);
        }
        x++;
			  if (++Diff == 8) {
          Diff = 0;
...
//          LCD_L0_SetPixelIndex(x, y, *(pTrans + pixel));
      ...
      for (; xsize > 0; xsize--, x++, p++) {
        ...
        if (pixel) {
//          LCD_L0_SetPixelIndex(x, y, pixel);
			LCD_Fast_DrawPoint(x,y,pixel);//画点函数
        }
      }
    }
  }
...
//  #endif
	///////////////////////////////////////////
  LCD_Fast_DrawPoint(x,y,PixelIndex);//画点函数
}
...
//  LCD_L0_SetPixelIndex(x, y, LCD_NUM_COLORS - PixelIndex - 1); //画点和填充一个点的颜色
	LCD_Fast_DrawPoint(x, y, LCD_NUM_COLORS - PixelIndex - 1);
}
...
void LCD_L0_DrawHLine  (int x0, int y,  int x1) {
  if (GUI_Context.DrawMode & LCD_DRAWMODE_XOR) {     //判断哪种模式
    for (; x0 <= x1; x0++) {
      LCD_L0_XorPixel(x0, y);
    }
  } else {
    for (; x0 <= x1; x0++) {
//      LCD_L0_SetPixelIndex(x0, y, LCD_COLORINDEX);  //画点和填充一个点的颜色
    LCD_Fast_DrawPoint(x0, y, LCD_COLORINDEX);
	}
  }
}
...
//      LCD_L0_SetPixelIndex(x, y0, LCD_COLORINDEX);
		LCD_Fast_DrawPoint(x, y0, LCD_COLORINDEX);
    }
  }
}
...
//  for (; y0 <= y1; y0++) {
//    LCD_L0_DrawHLine(x0, y0, x1);   //画行
//  }
	LCD_Fill(x0,y0,x1,y1,LCD_COLORINDEX);  //替换掉上面的代码可以加速刷屏速度
}
