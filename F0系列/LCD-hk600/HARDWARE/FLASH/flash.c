...
void FLASH_PageWrite(uint16_t *pBuffer, uint32_t WriteAddr)
{
  ...
  {
    FLASHStatus = FLASH_ProgramHalfWord(WriteAddr, *pBuffer);//一次写进16位数据，2字节
    WriteAddr += 2;  //一个地址存一个字节数据
    pBuffer++;
  }
}
...
void FLASH_WriteNWord(uint16_t *pBuffer, uint32_t WriteAddr, uint16_t nWord)
{
  ...
  uint32_t addr_first_page;                      //第一页(起始)地址,第几页开始
  ...
  length_beyond_start = ((WriteAddr % FLASH_PAGE_SIZE) / FLASH_TYPE_LENGTH);//页偏移地址
  length_remain_start = FLASH_PAGE_LENGTH - length_beyond_start;  //页剩余地址
  addr_first_page     = WriteAddr - (WriteAddr % FLASH_PAGE_SIZE);//得到页的开始地址，保持地址对齐

  /* 擦除(写操作)的页数 */
  if(0 == (length_beyond_start + nWord)%FLASH_PAGE_LENGTH)//如果刚好为<=512字节就进入
  {
    flag_last_page_fill = FLAG_OK;               //最后一页刚好
    NbrOfPage = (length_beyond_start + nWord) / FLASH_PAGE_LENGTH;//得到要写几页
  }
  else
  {
    flag_last_page_fill = FLAG_NOOK;             //···跨页
    NbrOfPage = (length_beyond_start + nWord) / FLASH_PAGE_LENGTH + 1;//加多一页
  }
  ...
  /* 操作第一页 */
  FLASH_ReadNWord(buf_tmp, addr_first_page, FLASH_PAGE_LENGTH); //取一页数据出来，防止开始地址
                                                            //不是页开始地址，整擦除页时丢掉之前的数据
  FLASHStatus = FLASH_ErasePage(addr_first_page); //擦除一页
  /* 只有1页 */
  if(1 == NbrOfPage)
  {
...
void FLASH_ReadNWord(uint16_t* pBuffer, uint32_t ReadAddr, uint16_t nWord)
{
  while(nWord--)
  {
    *pBuffer = (*(__IO uint16_t*)ReadAddr); //读取数据保存进pBuffer中
    ReadAddr += 2;
    pBuffer++;
  }
}
